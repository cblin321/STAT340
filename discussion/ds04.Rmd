...
The results match with the table.
...
permute = function(m) {
  est = c()
  for (i in 1:m) {
    xy = sample(mtcars.small)
    x = xy[, c("mpg")]
    y = xy[, c("drat")]
    # Fit the linear model
    model <- lm(x ~ y, data = mtcars.small)
    
    # Extract the coefficients and remove the names
    coefficients <- unname(coef(model))
    
    # The slope is the second element
