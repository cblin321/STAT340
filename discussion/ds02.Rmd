...
```{r}
  ...
  # TODO: code goes here.
right_swipe = subset(dating.population, attractiveness + personality >= 10)
cor(right_swipe$attractiveness, right_swipe$personality)
```
...
```{r}
  ...
  # TODO: code goes here.
right_swipe = subset(dating.population, attractiveness + personality >= 5)
cor(right_swipe$attractiveness, right_swipe$personality)
```
...
Attractive people tend could get dates easier because they are attractive, meaning they don't need to display their personality on their dating profile. But that doesn't mean they are jerks in reality.
...
# Create a subset of the population who are accepted to MSU and not accepted to ESU. This represents the freshman class of 2027.
  ...
  # TODO: code goes here.
  zStudents = data.frame(GPAs = zGPAs, SATs = zSATs)
  MSU_2027 = subset(zStudents, GPAs > 0 & SATs > 0)
```
...
```{r}
  ...
  # TODO: code goes here.
cor(MSU_2027$GPAs, MSU_2027$SATs)
...
Because SAT scores are not 1:1 correlated with GPA, so it would make sense that high SAT score students have lower SATs.
