...
<br/>
Christopher Lin, SEBASTIAN LAU, GABRIELLA LOUREIRO
...
#TODO: code goes here.
permutation_test = function(a, b, n) {
  observed = abs(mean(a) - mean(b))
  dist = c(a, b)
  reps = replicate(n, {
    shuf = sample(dist)
    a_shuf = shuf[1:length(a)]
    b_shuf = shuf[(length(b) + 1): length(dist)]
    a_shuf - b_shuf
  })  
  mean(reps >= observed) * 2
}
...
type_I_errors <- replicate(1e3, {
  sampleA = generate_Cauchydata(20, 10)
  sampleB = generate_Cauchydata(20, 10)
  
  test = permutation_test(sampleA, sampleB, 1e3)
