---
title: "Week 11 - Correlation, simple & multiple regression"
...
ggplot(mr, aes(log10(radius), log10(mass))) +
  geom_point() +
  xlab("Radius (Earth Radius)") +
  ylab("Mass (Earth Mass)") +
  #scale_x_log10() +
  #scale_y_log10() +
  geom_line(aes(x = log10(radius), y= ystar_plus_se), color = "green", linetype="dashed", size= 1.5) +
  geom_line(aes(x = log10(radius), y= ystar_minus_se), color = "green", linetype="dashed", size= 1.5) +
  geom_smooth(method="lm", se=TRUE, color="red")+
  geom_abline(aes(slope = coef(lm1)[2] , intercept = coef(lm1)[1]), color="blue") +
  geom_line(aes(x = log10(radius), y= y_plus_se), color = "red", linetype="dashed") +
  geom_line(aes(x = log10(radius), y= y_minus_se), color = "red", linetype="dashed") +
  geom_vline(aes(xintercept = (mean(log10(radius)))), color="blue", linetype="dotted") +
  theme(text = element_text(size=20))
```
...
- Previously we learned about linear regression when there is a response variable $Y$ and a single predictor variable $X$.  
-  More generally, you will encounter settings where there are multiple predictor variables $X_1$, $X_2$, ..., $X_p$.  
- In this setting, the linear model can be written as 

$$
Y_i = \beta_0 + \beta_1 X_{1i} + \cdots + \beta_pX_{pi} + \varepsilon_i
$$
The intercept $\beta_0$ and slope parameters $\beta_1, \ldots, \beta_p$ can be estimated using the method of least squares.  

We will use this model to predict finishing times of the runners in the 2013 Boston Marathon who were not able to finish due to the bombings.

# Recall:  Background on the Boston Marathon
...
- To run in the Boston Marathon, one must complete a marathon with a qualifying time within a particular time   
- In 2013 the Boston Marathon took place on April 15;  two bombs placed near the finish line exploded at 2:49 PM resulting in three deaths and several hundred injuries    
- Of less importance, 5,756 runners failed to finish the race  

> Question: The Boston Athletic Association (BAA) desired a procedure for projecting finishing times for the runners who could not complete the race in 2013 due to the bombings.

We are going to work toward predicting the finishing times for those runners failed to finish the race. 

To begin, we will look at some Boston Marathon data and do a little exploratory data analysis (EDA).  



### Understanding the data

- The data can be found at https://rls.sites.oasis.unc.edu/boston.html  
- The [Readme.pdf](https://rls.sites.oasis.unc.edu/faculty/rs/Bostonwebpage/Readme.pdf) file explains the variables in the *TIM.txt* file we will be working with.   
...
* StartHr, StartMin: 2013 participants only: the exact time the participant crossed the start line (StartHr is the hour, StatMin is minutes including decimal parts of a minute). This information will be used in some analyses to determine whether it was possible for the runner to have crossed the finish line before the bombs went off at 2:49 pm.
* K0-5, K5-10, through K40-Fin: Split time in decimal minutes for 5 km segments of the race plus a final shorter segment  
...
-  Adjust the following variables:  
  ...
  - `bib` = `BibNum` and make it a character type  
  - `year` = `Year`  
  - `age` = `Age`  
  - `sex` = `Gender1F2M`, change 1 to F and 2 to M  
  
```{r}
bm <- read_table("../../data/TIM.txt",
  ...
  mutate(bib = as.character(bib),
   ...
   mutate(final = pmap_dbl(select(., starts_with("K")), sum)) 
```


> How many runners in each year?
...
bm %>%
  count(year)
```
- The number of runners is fairly consistent among our three years of observations.  
- The 2012 Boston Marathon is not included in the analysis because it was a particularly hot year.  


> Big picture and overall goals

- We ultimately want to predict the finishing times of racers from 2013 who could not complete the marathon due to the bombings  
- We have data on racers from the 2010 and 2011 Boston Marathons that we can use for this  
- One idea will be to use a subset of the runners from 2010 and 2011 who are similar to the runners from 2013 who did not finish to build a model for the prediction  
- We will explore features of the data by year to assess if the 2010/2011 runners are similar enough to the 2013 runners so that we can build a model using the 2010/2011 results in order to predict the 2013 results.  



> Explore the Boston Marathon data

#### Side-by-side box plots of the finishing times by `sex`

```{r}
bm %>%
  group_by(year, sex) %>%
  summarize(med = median(final, na.rm=TRUE)) %>%
  pivot_wider(id_cols=year, names_from=sex, values_from=med)

ggplot(bm, aes(x=sex, y=final)) +
  geom_boxplot() +
  facet_wrap(~year) +
  ...
  ggtitle("Distribution of Boston Marathon Times") +
  geom_hline(yintercept = 215, color="red",linetype="dashed",size=1.5) +
  geom_hline(yintercept = 234, color="blue",linetype="dotted",size=1.5) +
  theme_bw() +
  theme(text = element_text(size=20))
```

- The distributions of finishing times between 2010 and 2011 are similar, but there is a clear difference compared to the 2013 finishing times  
-- This is because the 2013 finishing times include only the runners who finished before the bombs went off  
- The median overall time for females is slower than the median overall time for males.  
-  There are some outlier points both on the fast and slow ends of the distributions, but there is a longer tail toward the slower finishing times.  The range of the extremes is wider for the male runners;  that is, the male runners had the fastest and the slowest final times.  
 



#### Distribution of the age of runners on race day (violin plots)

```{r}
bm %>%
  group_by(year) %>%
  summarize(min = min(age), mean = mean(age), max = max(age))

## Distribution of Ages
ggplot(bm) +
  geom_violin(aes(y=age, x= 1),
                     color="black",fill="yellow",
              draw_quantiles = c(0.25, 0.5, 0.75),
              adjust = .5) +
  facet_wrap(~year) +
  ylab("Age on Race Day") +
  # xlab("Number of Runners") +
  ggtitle("Boston Marathon Runners", 
          subtitle = "Age distribution") +
  theme_bw() +
  theme(text = element_text(size=20))
```

- The youngest runners were 18 years old, the average ages were around 41, and the oldest runners were 81-83



#### Distribution of the age of runners by sex on race day (box plots)

```{r}
ggplot(bm, aes(x=sex, y=age, color = sex)) +
...
- The median female runner age was younger than the median male runner age.  
- The extreme ages appear similar between the male and female runners.  



#### Visualize the finishing times by age

```{r}
bm %>%
ggplot(aes(x=age, y=final, color=as.character(year))) +
  geom_point(alpha=.2) +
  geom_smooth(se=FALSE) +
  xlab("Age on Race Day") +
  ylab("Finishing Time (minutes)") +
  ggtitle("Boston Marathon Runners", 
          subtitle = "Finishing times by age") +
  labs(color="Year") +
  theme_bw() +
  theme(text = element_text(size=20))
```

- Observe that the smooth curves for 2010 and 2011 are similar, but the 2013 curve is systematically lower suggesting faster finishing times.



# Boston Runner Profiles

- Ultimately we want to predict the finishing times for the runners who did not complete the marathon in 2013 due to the bombings.  
- We will explore the runner profiles.  
-  The runner profiles indicate the split times for the runners as they move through the course.  
-  The split times are provided in 5 KM intervals (except for the final 2.2 KM stretch)  


First we need to get the split interval labels in a form that is easier for plotting and keeps the splits in the correct sequence.

```{r}
bm_long <- bm %>%
  pivot_longer(cols= starts_with("K"), names_to="split", values_to="split_time", names_prefix="K") %>%
  separate(split, c("split_start","split_end")) %>%
  mutate(split_start = as.numeric(split_start),
         split_end = as.numeric(split_end))
bm_long
```



##### Pick a runner and plot the runner's pace across splits
...
bm_long %>%
  # filter(split_start < 40 ) %>%
  filter(year==year_bib, bib %in% runners) %>%
  ggplot(aes(x=split_start, y=split_time, color=bib)) +
  geom_point() +
  geom_line() +
  xlab("Split Start (KM)") +
  ylab("Split time (minutes)") +
  labs(color="Bib Number") +
  ggtitle("Boston Marathon",
          subtitle = str_c("Runner Profiles in ", year_bib)) +
  theme_bw() +
  theme(text = element_text(size=20))
```

- The final leg of the race is only 2.2 KM compared to 5 KM for the other legs, which contributes to why we see a significant drop with the final point.  
- We can remove the final leg from the plot to improve the visualization


##### More runner profiles

```{r}
runners <- c(2000:2005, 10000:10005)
...
##### Next we look at the average split times by sex for the runners.

```{r}
bm_long %>%
  filter(split_start < 40, year==year_bib) %>%
  group_by(sex, split_start) %>%
  summarize(mean = mean(split_time), sd = sd(split_time)) %>%
  ggplot(aes(x=split_start, y=mean, color = sex)) +
  geom_point() +
  geom_line(size = 1.5) +
  geom_line(aes(y=mean+sd),linetype="dashed") +
  geom_line(aes(y=mean-sd),linetype="dashed") +
  geom_ribbon(aes(ymin=mean-sd,ymax=mean+sd, fill=sex), alpha=0.2, show.legend = FALSE) +
  xlab("Split Start (KM)") +
  ylab("Average Split time (minutes)") +
  labs(color="Sex") +
  ggtitle("Boston Marathon",
          subtitle = str_c("Average Runner Profiles by Sex in ", year_bib)) +
  theme_bw() +
  theme(text = element_text(size=20))
```


Next, plot the average split times by year and by sex.

```{r}
bm_long %>%
  filter(split_start < 40 ) %>%
  group_by(year, sex, split_start) %>%
  summarize(mean = mean(split_time, na.rm=TRUE), sd = sd(split_time, na.rm=TRUE)) %>%
  ggplot(aes(x=split_start, y=mean, color = as.character(year), linetype=sex)) +
  geom_point() +
  geom_line(size = 1.25) +
  xlab("Split Start (KM)") +
  ylab("Average Split time (minutes)") +
  ...
  ggtitle("Boston Marathon",
          subtitle = "Average Runner Profiles by Year and Sex")+
  theme_bw() +
  theme(text = element_text(size=20))
```


Add +/- standard deviations bands for the Female runners
...
bm_long %>%
  ...
  geom_point() +
  geom_line(size = 1.5) +
  geom_ribbon(aes(ymin=mean-sd,ymax=mean+sd, fill=year), alpha=0.2, show.legend = FALSE) +
  xlab("Split Start (KM)") +
  ylab("Average Split time (minutes)") +
  labs(color="Year", linetype = "Sex") +
  ggtitle("Boston Marathon",
          subtitle = "Average Runner Profiles by Year for Females") +
  theme_bw() +
  theme(text = element_text(size=20))
```
...
Add +/- standard deviations bands for the Male runners
...
bm_long %>%
  ...
  labs(color="Year", linetype = "Sex") +
  ggtitle("Boston Marathon",
          subtitle = "Average Runner Profiles by Year for Males") +
  theme_bw() +
  theme(text = element_text(size=20))
```

- When reviewing the average runner profiles, there is quite a lot of variability around the means.  
...
# Selecting our data for prediction

- Recall that we want to predict the finishing times among the 5,756 runners who were not able to complete the 2013 race due to the bombings.  
-  We need to identify the runners who did not complete the race due to the bombings, so we'll need a few extra points of information.  
- Runners start the marathon in waves, and the third and final wave started at 10:40 AM.  
- The bombs went off at 2:49 PM.  

```{r}
## Find latest recorded start time from 2013
bm %>%
  filter(year == 2013) %>%
  arrange(desc(StartHr), desc(StartMin))

library(lubridate)
hm("14:49")-hms("10:53:08")
```  
- Based on the latest recorded start time (10:53:06 AM), any runner who had a finishing time under 3 hours and 56 minutes would have completed the race before the bombings.  
- This means that runners who may not have completed the race due to the bombings would generally have finishing times greater than 4 hours.  



> Do the profiles look different for runners who complete the race in over 4 hours compared to those who complete it in under 4 hours?

```{r}
bm_long %>%
  filter(split_start < 40 ) %>%
  mutate(four = ifelse(final < 4*60, "< 4 hours", ">= 4 hours")) %>%
  group_by(four, year, sex, split_start) %>%
  summarize(mean = mean(split_time, na.rm=TRUE), sd = sd(split_time, na.rm=TRUE)) %>%
  drop_na(four) %>%
ggplot(aes(x=split_start, y=mean, color = as.character(year), linetype=sex)) +
  geom_point() +
...
- There are differences between the profiles for runners who finish in under 4 hours compared to those who take over 4 hours (aside from just slower times).  
-  It appears that runners who take over 4 hours have split times that increase throughout the course, on average, more than for the runners who finish in under 4 hours.  
-  This implies we should restrict our attention to runners from our data who completed the race in over 4 hours.  
-  We will also exclude the 2013 runners who did not finish but they quit before the half-way point because this may not due to the bombings.  


##### Below we define our data set to be used in building our prediction model.  

```{r}
## Select runners for analysis
data0 <- bm %>%
  filter(final >= 4*60 | (is.na(final) & !is.na(HalfMar) & year == 2013)) 

nrow(data0)
```

- Our training data have 21941 runners. 


##### Add an indicator, `dnf`, for the runners who did not finish the race in 2013.

```{r}
## Add indicator for 2013 non-finishers
data0 <- data0 %>%
  mutate(dnf = ifelse(is.na(final), TRUE, FALSE))
...
- There are 5,628 runners from 2013 in our reduced data who did not finish the race.  
-  We will try to predict their finishing times using the remaining observations from `data0`.


##### Now let's get our data in the longer form so it is easier to plot the profiles.
...
>  Of the DNF's, when did they stop the race?

```{r}
data %>%
  filter(dnf & is.na(split_time)) %>%
  group_by(bib) %>%
  slice_min(n=1, order_by=split_start) %>%
  ungroup() %>%
  group_by(split_start) %>%
  summarize(n = n(), Percent = round(n/5628*100, 1))

```

- Notice that 80% of the runners nearly completed the race and only had the final 2.2 KM stretch to go.



#### Compare the average split times

We will begin by predicting the finishing times for those 2013 runners who only missed the final split K40-Fin.  

Let's visualize the average differences between the training sample and the 2013 runners up to their missing split.

```{r}
data %>%
  # filter(split_start < 40 ) %>%
  group_by(dnf, split_start) %>%
  summarize(mean = mean(split_time, na.rm=TRUE), sd = sd(split_time, na.rm=TRUE)) %>%
  ggplot(aes(x=split_start, y=mean, color = dnf)) +
  geom_point(show.legend = FALSE) +
  geom_line(size = 1.5,show.legend = FALSE) +
  geom_ribbon(aes(ymin=mean-sd,ymax=mean+sd, fill=dnf), alpha=0.2, show.legend = FALSE) +
  xlab("Split Start (KM)") +
  ylab("Average Split time (minutes)") +
  # labs(color="Missing final split") +
  ggtitle("Boston Marathon",
          subtitle = "Average Runner Profiles") +
  theme_bw() +
  theme(text = element_text(size=20))
```
...
- There are a number of different approaches for carrying out the prediction of the finishing times for the runners who did not complete the 2013 Boston Marathon due to the bombings.  
-  The paper [*Completing the Results of the 2013 Boston Marathon*](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0093800) by Hammerling et al (2014) in PLOS ONE tries several approaches for carrying out the prediction task.  For example, a couple of the methods are listed below. 
    1. Linear regression  
    2. Nearest Neighbor (to select the 200 runner profiles that are most similar to the 2013 runner that needs a predicted final time) + local linear regression  




We will build several regression models using the runners in our reduced data set, `data`, who did finish the race using the equation below.  

- The model will be built using a subset of $1:J$ splits to predict the finishing time:

$$
y_i = \beta_0 + \sum_{j = 1}^J x_{ji}\beta_j + \varepsilon_i
$$

- $y_i$ = the final time for runner $i$  
- $J$ = number of available split times  
- $x_{ji}$ = the available split time for runner $i$'s interval $j$  
- $\varepsilon_i$ = random error for runner $i$, assumed to satisfy $E(\varepsilon_i)=0$, and the $\varepsilon_i$'s are considered uncorrelated with a constant variance of $\sigma^2$.  

We will estimate the $\beta_j$'s using the `dnr = FALSE` data, and then predict the finishing times for the `dnr = TRUE` runners using the estimated $\beta_j$'s to fill in the missing values.


First we rename the variables corresponding to the intervals to make it a little easier to specify our models:
...
Below we fit five linear models.  

Each linear model has a different drop-out point for the runners where we use the previous split times to estimate the final split time.

Let's focus on the first model defined in the chunk below using the first 8 intervals:

$$
y_i = \beta_0 + \sum_{j = 1}^8 x_{ij}\beta_j + \varepsilon_i = \beta_0 + x_{i1}\beta_1 + \cdots + x_{i8}\beta_8 + \varepsilon_i
$$

```{r}
fit35 <- lm(final~k0+k5+k10+k15+k20+k25+k30+k35, data = train)
summary(fit35)
```

- `fit35` is a linear model that uses all the split times except for the final 2.2 KM leg of the race.  
-  Notice that we have multiple covariates in this model.  Earlier in the semester we only considered linear models with a single covariate.  To include multiple covariates, we use the `lm()` function and add them to the right of the `~`.   
-  The summary of the results indicates which of the estimated slopes are statistically significantly different from 0.  
- In `fit35`, all the estimated coefficients have very small p-values.  
- Most of the estimated slopes are near 1.  This makes sense because if we had included all of the intervals, the true model would be the sum of the interval times (so all the slopes would be 1).


Let's take a peek at the residuals.  Since we have multiple covariates, we will plot the predicted finishing times on the x-axis.  

```{r}
## Predicted vs. observed final times
train %>%
  add_residuals(fit35) %>%
  add_predictions(fit35) %>%
  ggplot(aes(x = final, y=pred)) +
  geom_point() +
  xlab("Finishing time (minutes)") +
  ylab("Predicted finishing times (minutes)") 
...
train %>%
  ...
  # ylim(-30, 30) +
  xlab("Predicted finishing times (minutes)") +
  ylab("Residuals from fit35 (minutes)") +
  ggtitle("Boston Marathon Linear Model Residuals",
          subtitle = "2010, 2011, 2013 Training Data") +
  theme_bw() +
  theme(text = element_text(size=20))


## Explore the large residual
train %>%
  add_residuals(fit35) %>%
  add_predictions(fit35) %>%
  arrange(desc(resid))
...
It looks like the residual is from a runner who had an unusual split in the final 2.2 km.  The runner took 202 minutes to complete the final split, with an overall time of 389 minutes...this means over half their time was spent on about 5% of the total distance.  Something unusual happened to this runner (maybe an injury?), so we can remove this runner and refit the model.

```{r}
## Remove the residual
train_2 = train %>%
  filter(index != 14707)
fit35_2 <- lm(final~k0+k5+k10+k15+k20+k25+k30+k35, data = train_2)
summary(fit35_2)

## Minor changes to the estimated coefficients:
rbind(coef(fit35), coef(fit35_2))
```

Now we can predict the finishing times for the runners from 2013 who did not finish the race, but completed all but the 40-Finish leg of the race.

First we need to select the runners that fit in this group:
...
which_runners <- data %>%
  ...
  group_by(bib) %>%
  slice_min(n=1, order_by=split_start) %>%
  filter(split_start == 40) %>% #only missing final split
  pull(bib)


test35 <- dnf %>%
  filter(bib %in% which_runners) %>%
  select(bib, starts_with("k"))
test35
```
Now we can use the `fit35` model to predict the finishing times:

```{r}
test35 <- test35 %>%
  mutate(final = predict(fit35_2, test35),
         predict = TRUE)
test35
```
...
Check if the predicted times seem reasonable compared to training data
```{r}
pred35 = train %>%
  mutate(predict = FALSE) %>%
  bind_rows(test35) 

ggplot(pred35, aes(x = predict, y = final)) +
  geom_boxplot() +
  xlab("Predicted finishing time indicator") +
  ylab("Final time") +
  theme_bw() +
  theme(text = element_text(size=20))
```

The two distributions *appear* reasonably consistent...can we detect differences in the mean of these distributions?

If we view these as two independent samples from our population of potential runners of the Boston Marathon, we can carry out a hypothesis test to see if we detect a difference between the mean finishing times using the samples defined above.   
...
### Use t.test to compute a p-value
t.test(group1, group2)

### Manually compute the test statistic
tstat = (mean(group1) - mean(group2))/sqrt(var(group1)/n1 + var(group2)/n2)
tstat

pt(tstat, t.test(group1, group2)$parameter)*2
```

This test suggests there is a statistically significant difference between the mean of the predicted finishing times and the mean of the training finishing times.  However, the estimated difference in the means is under 5 minutes.  Is this practically significant for the population of runners who would take over 4 hours to complete the Boston Marathon?  


Below we fit the remaining four linear models.  These models can be used to predict the finishing times for the 2013 runners who dropped out after (i) the 35 KM mark, (ii) the 30 KM mark, (iii) the 25 KM mark, or (iv) the 20 KM mark.

```{r}
fit30 <- lm(final~k0+k5+k10+k15+k20+k25+k30, data = train_2)
summary(fit30)

fit25 <- lm(final~k0+k5+k10+k15+k20+k25, data = train_2)
summary(fit25)

fit20 <- lm(final~k0+k5+k10+k15+k20, data = train_2)
summary(fit20)

fit15 <- lm(final~k0+k5+k10+k15, data = train_2)
summary(fit15)
```

- You can work on evaluating the residuals for these models and predicting the finishing times for the corresponding 2013 runners on your own if you like!

- Can you think of other variables to include in the linear model?
